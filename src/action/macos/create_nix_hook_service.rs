...
//         let launchd_service_path = home_dir().unwrap().display().to_string() + "/Library/LaunchAgents/systems.determinate.nix-installer.nix-hook.plist";
//         let mut this = Self {
...
//     let log_err_file_path = format!("{}/Library/Logs/nix-installer.err.log", home_dir().unwrap().display().to_string());
//     let log_out_file_path = format!("{}/Library/Logs/nix-installer.log", home_dir().unwrap().display().to_string());
